...
/* Signals generated by SR stage and used in prev. stages */
int sr_reg_data, 
	sr_n, sr_z, sr_p,
	v_sr_ld_cc,
	v_sr_ld_reg,
...
/* Signals generated by MEM stage and used in prev. stages */
int mem_pcmux = 0, target_pc = 0, trap_pc = 0; /* sent to fetch */
int v_mem_ld_cc = 0, v_mem_ld_reg = 0; /* sent to decode */
int cc_match = 0; /* send to fetch; determine if branch was hit or miss */
/* Signals generated by AGEX stage and used in prev. stages */
int v_agex_ld_cc = 0, v_agex_ld_reg = 0; /* sent to decode */
...
void MEM_stage() {
	...
	cc_match = ((cc & 0x1) && P) || ((cc & 0x2) && Z) || ((cc & 0x4) && N);
	if (PS.MEM_V) {
		if (Get_BR_OP(addr)) {
			if (cc_match) { mem_pcmux = 1; }
			else { mem_pcmux = 0; }
		}
		else if (Get_UNCOND_OP(addr)) {
			int jsr = (PS.MEM_IR >> 11) & 1;
			mem_pcmux = 1;
		}
		else if (Get_TRAP_OP(addr)) { mem_pcmux = 2; }
		else { mem_pcmux = 0; }
	}
	/* Logic Box of Doom */
	v_mem_ld_reg = 0;
	...
	v_mem_ld_cc = 0;
	if (PS.MEM_V) {
		if (Get_MEM_LD_CC(addr)) { v_mem_ld_cc = 1; }
		if (Get_MEM_LD_REG(addr)) { v_mem_ld_reg = 1; }
		if (Get_MEM_BR_STALL(addr)) { v_mem_br_stall = 1; }
	}
	/* Memory Operations + MEM.STALL, TARGET.PC, TRAP.PC*/
	int mem_en = Get_DCACHE_EN(addr) && PS.MEM_V;
	int data_out = 0, data_in = 0, dcache_r = 0;
	if (mem_en) {
		if (Get_DCACHE_RW(addr)) {
			/* Write Data */
			data_in = PS.MEM_ALU_RESULT;
			if (Get_DATA_SIZE(addr)) {
				dcache_access(PS.MEM_ADDRESS, &data_out, data_in, &dcache_r, 1, 1);
			} else {
				/* Top Byte (1), Low Byte (0) */
				if (PS.MEM_ADDRESS & 0x1) {
					data_in = (data_in << 8) & 0xFFFF;
					dcache_access(PS.MEM_ADDRESS, &data_out, data_in, &dcache_r, 0, 1);
					data_out = Low16bits(SEXT((data_out & 0xFF00) >> 8, 8));
				} else {
					dcache_access(PS.MEM_ADDRESS, &data_out, data_in, &dcache_r, 1, 0);
					data_out = Low16bits(SEXT(data_out & 0xFF, 8));
				}
			}
		} else {
			/* Read Data */
			dcache_access(PS.MEM_ADDRESS, &data_out, 0, &dcache_r, 0, 0);
			if (!Get_DATA_SIZE(addr)) {
				/* Top Byte (1), Low Byte (0) */
				if (PS.MEM_ADDRESS & 0x1) { data_out = Low16bits(SEXT((data_out & 0xFF00) >> 8, 8)); }
				else { data_out = Low16bits(SEXT(data_out & 0xFF, 8)); }

			}
		}
	}
	mem_stall = mem_en && !dcache_r;
	...
	/* Signal to SR.ADDRESS, SR.NPC, SR.ALU.RESULT, SR.IR, SR.DRID*/
	...
	/* Signal to SR.V */
	if (PS.MEM_V == 0 || mem_stall) { NEW_PS.SR_V = 0; }
	else { NEW_PS.SR_V = 1; }
}
...
void AGEX_stage() {
	int* addr = PS.AGEX_CS;
	/* Create Logic for V.AGEX.LD.CC, V.AGEX.LD.REG, V.AGEX.BR.STALL */
	v_agex_br_stall = 0;
	v_agex_ld_reg = 0;
	v_agex_ld_cc = 0;
	if (PS.AGEX_V) {
		if (Get_AGEX_LD_CC(addr)) { v_agex_ld_cc = 1; }
		if (Get_AGEX_LD_REG(addr)) { v_agex_ld_reg = 1; }
		if (Get_AGEX_BR_STALL(addr)) { v_agex_br_stall = 1; }
	}
	/* Generate LD.MEM Signal */
	int LD_MEM = 1;
	if (mem_stall) { LD_MEM = 0; }
	/* Latch Values to MEM */
	if (LD_MEM) {
		/* Signal to MEM.ADDRESS */
		int mem_addr;
		if (Get_ADDRESSMUX(addr)) {
			/* Compute addr1mux_out and addr2mux_out */
			int addr1mux_out, addr2mux_out;
			if (Get_ADDR1MUX(addr)) { addr1mux_out = PS.AGEX_SR1; }
			else { addr1mux_out = PS.AGEX_NPC; }
			int addr2mux = Get_ADDR2MUX(addr);
			if (addr2mux == 0) { addr2mux_out = 0; }
			else if (addr2mux == 1) { addr2mux_out = SEXT(PS.AGEX_IR & 0x3F, 6); }
			else if (addr2mux == 2) { addr2mux_out = SEXT(PS.AGEX_IR & 0x1FF, 9); }
			else if (addr2mux == 3) { addr2mux_out = SEXT(PS.AGEX_IR & 0x7FF, 11); }
			if (Get_LSHF1(addr)) { addr2mux_out = addr2mux_out << 1; }
			/* Add and latch address*/
			mem_addr = Low16bits(addr1mux_out + addr2mux_out);
		}
		else { mem_addr = ZEXT(PS.AGEX_IR & 0xFF, 8) << 1; }
		NEW_PS.MEM_ADDRESS = mem_addr;
		/* Signal to MEM.ALU.RESULT */
		int a, b, out;
		a = PS.AGEX_SR1;
		if (Get_SR2MUX(addr) == 1) { b = SEXT((PS.AGEX_IR & 0x1F), 5); }
		else { b = PS.AGEX_SR2; }
		if (Get_ALU_RESULTMUX(addr)) {
			/* ALU: ADD, AND, XOR, PASSA */
			int aluk = Get_ALUK(addr);
			if (aluk == 0) { out = Low16bits(a + b); }
			else if (aluk == 1) { out = Low16bits(a & b); }
			else if (aluk == 2) { out = Low16bits(a ^ b); }
			else if (aluk == 3) { out = Low16bits(b); }
		} else {
			/* SHF: left, right logical, right arithmetic */
			int shf_amt = PS.AGEX_IR & 0xF;
			int right = PS.AGEX_IR & 0x10;
			int arith = PS.AGEX_IR & 0x20;
			if (right) {
				if (arith) { out = Low16bits(RSHF(a, shf_amt, (a >> 15) & 0x1)); } /* Right Arithmetic */
				else {out = Low16bits(RSHF(a, shf_amt, 0)); } /* Right Logical */
			} else { out = Low16bits(a << shf_amt); } /* Left Logical */
		}
		NEW_PS.MEM_ALU_RESULT = out;
		/* Signal to MEM.IR, MEM.DRID, MEM.NPC, MEM.CC */
...
/************************* DE_stage() *************************/
void DE_stage() {
	/* Get Control_Store_Address */
	int CONTROL_STORE_ADDRESS = ((NEW_PS.DE_IR & 0xF800) >> 10)
		+ ((NEW_PS.DE_IR & 0x20) >> 5);
	/* Compute Reg File Signals and Reg File Write */
	int sr1_reg, sr2_reg, sr1, sr2;
	sr1_reg = (PS.DE_IR & 0x1C0) >> 6;
	sr1 = REGS[sr1_reg];
	if (PS.DE_IR & 0x2000) {
		sr2_reg = (PS.DE_IR & 0xE00) >> 9;
		sr2 = REGS[sr2_reg];
	} else {
		sr2_reg = PS.DE_IR & 0x7;
		sr2 = REGS[sr2_reg];
	}
	if (v_sr_ld_reg) { REGS[sr_reg_id] = sr_reg_data; }
	/* Dependency Check Logic */
	dep_stall = 0;
		/* Step 1: Check register dependencies */
	int sr1_needed = CONTROL_STORE[CONTROL_STORE_ADDRESS][0];
	int sr2_needed = CONTROL_STORE[CONTROL_STORE_ADDRESS][1];
	if (sr1_needed) {
		if ( ((PS.AGEX_DRID == sr1_reg) && (v_agex_ld_reg))
			|| ((PS.MEM_DRID == sr1_reg) && (v_mem_ld_reg))
			|| ((PS.SR_DRID == sr1_reg) && (v_sr_ld_reg))
			) { dep_stall = 1; }
	}
	if (sr2_needed) {
		if ( ((PS.AGEX_DRID == sr2_reg) && (v_agex_ld_reg))
			|| ((PS.MEM_DRID == sr2_reg) && (v_mem_ld_reg))
			|| ((PS.SR_DRID == sr2_reg) && (v_sr_ld_reg))
			) { dep_stall = 1; }
	}
		/* Step 2: Check CC dependencies */
	if (CONTROL_STORE[CONTROL_STORE_ADDRESS][12]) {
		if (v_agex_ld_cc || v_mem_ld_cc || v_sr_ld_cc) { dep_stall = 1; }
	}
	/* Branch Stall Logic */
	v_de_br_stall = 0;
	if (PS.DE_V) {
		if (CONTROL_STORE[CONTROL_STORE_ADDRESS][15] || PS.DE_IR == 0xF025) {
			v_de_br_stall = 1;
		}
	}
	/* Generate LD_AGEX Signal */
	int LD_AGEX = 1;
	if (mem_stall) { LD_AGEX = 0; }
	/* Latch AGEX Values */
	if (LD_AGEX) {
		/* Signal to AGEX.SR1, AGEX.SR2, AGEX.IR, AGEX.NPC */
		NEW_PS.AGEX_SR1 = sr1;
		NEW_PS.AGEX_SR2 = sr2;
		NEW_PS.AGEX_IR = PS.DE_IR;
		NEW_PS.AGEX_NPC = PS.DE_NPC;
		/* Signals from CONTROL STORE to AGEX.CS latch. */
		int ii = 0, jj = 0;
		for (ii = COPY_AGEX_CS_START; ii< NUM_CONTROL_STORE_BITS; ii++) {
			NEW_PS.AGEX_CS[jj++] = CONTROL_STORE[CONTROL_STORE_ADDRESS][ii];
		}
		/* Signal to AGEX.CC */
		if (N == 1) { NEW_PS.AGEX_CC = 4; }
		else if (P == 1) { NEW_PS.AGEX_CC = 1; }
		else { NEW_PS.AGEX_CC = 2; }
		if (Get_SR_LD_CC(PS.SR_CS) && PS.SR_V) {
			N = sr_n;
			Z = sr_z;
			P = sr_p;
		}
		/* Signal to AGEX.DRID */
		if (CONTROL_STORE[CONTROL_STORE_ADDRESS][2]) { NEW_PS.AGEX_DRID = 7; }
		else { NEW_PS.AGEX_DRID = (PS.DE_IR & 0xE00) >> 9; }
		/* Signal to AGEX.V */
		if (PS.DE_V == 0 || dep_stall) { NEW_PS.AGEX_V = 0; }
...
void FETCH_stage() {
	/* fetch instruction */
	int DE_IR_temp = 0;
	icache_access(PC, &DE_IR_temp, &icache_r);
	/* Generate LD_DE Signal */
	int LD_DE = 1;
	...
	int de_valid;
	if (LD_DE) {
		NEW_PS.DE_IR = DE_IR_temp;
		NEW_PS.DE_NPC = PC + 2;
		if (v_de_br_stall || v_agex_br_stall
			|| v_mem_br_stall || !icache_r) { de_valid = 0; }
		else { de_valid = 1; }
		NEW_PS.DE_V = de_valid;
	}
	/* Update PC */
	int no_stall = !v_de_br_stall && !v_agex_br_stall && !v_mem_br_stall
		&& !dep_stall && !mem_stall && icache_r;
	int jump_op = PS.MEM_V && Get_UNCOND_OP(PS.MEM_CS);
	int br_op = PS.MEM_V && Get_BR_OP(PS.MEM_CS);
	int trap_op = PS.MEM_V && Get_TRAP_OP(PS.MEM_CS);
	int ld_pc = 0;
	if (no_stall || trap_op || jump_op) { ld_pc = 1; }
	else if (br_op && cc_match) { ld_pc = 1; }
	if (ld_pc) {
		if (mem_pcmux == 0) { PC += 2; }
		else if (mem_pcmux == 1) { PC = target_pc; }
		else if (mem_pcmux == 2) { PC = trap_pc; }
	}
}